...
function displayStructureReview() {
    ...
    subjectsTopicsDisplay.innerHTML = subjects.map((subject, idx) => {
...
if (generateSubjectsBtn) {
    ...
    try {
        ...
        structureStatus.textContent = `✓ Generated ${courseStructure.subjects.length} subjects - Ready for lesson & question generation`;
        showToast('Course structure generated! You can now generate lessons or switch to QBank tab to create questions.', 'success');
    } catch (error) {
...
function formatLessonContent(content, chapters = null, topicId = null) {
    ...
    // Extract and process Mermaid code blocks first
    const mermaidBlocks = [];
